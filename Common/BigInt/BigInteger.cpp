...
BigInteger::BigInteger(const Blk *b, Index blen, Sign s) : mag(b, blen) {
	...
	case zero:
		if (!mag.isZero())
			maPanic(-1, "BigInteger::BigInteger(const Blk *, Index, Sign): Cannot use a sign of zero with a nonzero magnitude");
		sign = zero;
		break;
	case positive:
	case negative:
	...
	default:
		...
		maPanic(-1, "BigInteger::BigInteger(const Blk *, Index, Sign): Invalid sign");
	}
}
...
BigInteger::BigInteger(const BigUnsigned &x, Sign s) : mag(x) {
	...
	case zero:
		if (!mag.isZero())
			maPanic(-1, "BigInteger::BigInteger(const BigUnsigned &, Sign): Cannot use a sign of zero with a nonzero magnitude");
		sign = zero;
		break;
	case positive:
	case negative:
	...
	default:
		/* g++ seems to be optimizing out this case on the assumption
		 * that the sign is a valid member of the enumeration.  Oh well. */
		maPanic(-1, "BigInteger::BigInteger(const BigUnsigned &, Sign): Invalid sign");
	}
}
...
X BigInteger::convertToUnsignedPrimitive() const {
	if (sign == negative)
		maPanic(-1, "BigInteger::to<Primitive>: "
...
BigInteger::CmpRes BigInteger::compareTo(const BigInteger &x) const {
	...
	default:
		maPanic(-1, "BigInteger internal error");
	}
}
...
void BigInteger::divideWithRemainder(const BigInteger &b, BigInteger &q) {
	...
	if (this == &q)
		maPanic(-1, "BigInteger::divideWithRemainder: Cannot write quotient and remainder into the same variable");
	if (this == &b || &q == &b) {
		BigInteger tmpB(b);
