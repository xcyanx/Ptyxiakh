...
void BigUnsigned::subtract(const BigUnsigned &a, const BigUnsigned &b) {
	...
	if (b.len == 0) {
		...
		maPanic(-1, "BigUnsigned::subtract: "
	...
	if (borrowIn) {
		len = 0;
		maPanic(-1, "BigUnsigned::subtract: Negative result in unsigned calculation");
	} else
...
void BigUnsigned::divideWithRemainder(const BigUnsigned &b, BigUnsigned &q) {
	...
	if (this == &q)
		maPanic(-1, "BigUnsigned::divideWithRemainder: Cannot write quotient and remainder into the same variable");
...
void BigUnsigned::bitShiftLeft(const BigUnsigned &a, int b) {
	...
	if (b < 0) {
		if (b << 1 == 0)
			maPanic(-1, "BigUnsigned::bitShiftLeft: "
...
void BigUnsigned::bitShiftRight(const BigUnsigned &a, int b) {
	...
	if (b < 0) {
		if (b << 1 == 0)
			maPanic(-1, "BigUnsigned::bitShiftRight: "
				"Pathological shift amount not implemented");
		else {
			bitShiftLeft(a, -b);
			return;
...
void BigUnsigned::operator --() {
	if (len == 0)
		maPanic(-1, "BigUnsigned::operator --(): Cannot decrement an unsigned zero");
	Index i;
	bool borrow = true;
	for (i = 0; borrow; i++) {
		borrow = (blk[i] == 0);
